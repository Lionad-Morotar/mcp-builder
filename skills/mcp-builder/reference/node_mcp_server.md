...
- **不推荐**：旧的已弃用 API，如 `server.tool()`、`server.setRequestHandler(ListToolsRequestSchema, ...)` 或手动处理器注册
...
6. **错误处理**：始终使用 try-catch 进行适当的错误类型检查
7. **空安全**：使用可选链（`?.`）和空值合并（`??`）
...
- [ ] 所有工具使用 Zod 模式进行运行时输入验证，并使用 `.strict()` 强制执行
